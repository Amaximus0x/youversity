...
<header class="w-full flex flex-col">
  ...
  <div class="w-full bg-brand-red py-2 px-4 text-center">
    <p class="text-white text-sm md:text-base">if it's on YouTube, you can master it on YouVersity.</p>
  </div>
  
  <!-- Main Header -->
  <div class="w-full bg-dark-bg-primary py-4 px-4 md:px-8">
    <div class="max-w-6xl mx-auto flex justify-between items-center">
      <!-- Logo -->
      <div class="flex items-center">
        <a href="/" class="flex items-center">
          <img src="/YV.svg" alt="Youversity Logo" class="w-[35px] h-[38px] md:w-[44.97px] md:h-[48.43px] mr-2" />
          <div class="pt-2">
            <svg
              class="w-[86.61px] h-[16.76px] md:w-[107.83px] md:h-[21.36px] text-white"
              viewBox="0 0 109 24"
              fill="none"
      ...
      <div class="hidden md:flex items-center space-x-8">
        <nav class="flex items-center space-x-6">
          <a 
            href="/" 
            class="text-white hover:text-brand-red transition-colors font-medium {$page.url.pathname === '/' ? 'text-brand-red' : ''}"
          >
            Home
          </a>
          <a 
            href="/about" 
            class="text-white hover:text-brand-red transition-colors font-medium {$page.url.pathname === '/about' ? 'text-brand-red' : ''}"
          >
            About
          </a>
          <a 
            href="/contact" 
            class="text-white hover:text-brand-red transition-colors font-medium {$page.url.pathname === '/contact' ? 'text-brand-red' : ''}"
          >
            Contact
          </a>
        </nav>
        ...
        <button 
          ...
          class="px-6 py-2 bg-brand-red text-white rounded-lg hover:bg-opacity-90 transition-all font-medium"
        >
          Sign in
        </button>
      </div>
  ...
  {#if isMenuOpen && isMobile}
    <div class="md:hidden bg-dark-bg-primary border-t border-dark-border">
      <nav class="flex flex-col px-4 py-2">
        <a 
          href="/" 
          class="py-2 text-white hover:text-brand-red transition-colors font-medium {$page.url.pathname === '/' ? 'text-brand-red' : ''}"
        ...
        <a 
          href="/about" 
          class="py-2 text-white hover:text-brand-red transition-colors font-medium {$page.url.pathname === '/about' ? 'text-brand-red' : ''}"
          on:click={() => isMenuOpen = false}
        >
          About
        </a>
        <a 
          href="/contact" 
          class="py-2 text-white hover:text-brand-red transition-colors font-medium {$page.url.pathname === '/contact' ? 'text-brand-red' : ''}"
          on:click={() => isMenuOpen = false}
        >
          Contact
        </a>
