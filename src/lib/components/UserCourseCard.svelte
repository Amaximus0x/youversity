<script lang="ts">
    ...
    async function handleNavigateToCourse(courseId: string) {
        try {
            if ($user) {
                // Get enrollment progress to determine current module
                const enrollmentProgress = await getEnrollmentProgress($user.uid, courseId);
                
                // Set current module based on progress
                let currentModule = 0; // Default to first module
                
                if (enrollmentProgress?.lastAccessedModule !== undefined) {
                    // If there's a last accessed module and it's not the intro (-1)
                    currentModule = enrollmentProgress.lastAccessedModule >= 0 
                        ? enrollmentProgress.lastAccessedModule 
                        : 0;
                } else if (enrollmentProgress?.completedModules?.length) {
                    // If no last accessed module but has completed modules
                    // Set to the next uncompleted module or last module
                    const lastCompleted = Math.max(...enrollmentProgress.completedModules);
                    currentModule = Math.min(lastCompleted + 1, course.Final_Module_Title.length - 1);
                }
                
                // Update the store and navigate
                currentModuleStore.set(currentModule);
