<script lang="ts">
  ...
  onMount(async () => {
    ...
    if (storedLoadingState) {
      ...
      if (state.isLoading && !state.isInitialBuild) {
        loadingState.startLoading(state.courseTitle, true, false);
        loadingState.setProgress(state.progress);
      }
    }
...
<!-- Global modal that persists across all pages - moved outside all conditions -->
<div class="fixed-overlay">
  <CourseGenerationModal />
</div>

{#if !$page?.url?.pathname?.startsWith('/login')}
  <div class="flex h-screen overflow-hidden bg-light-bg-primary dark:bg-dark-bg-primary">
    {#if $page?.url?.pathname !== '/create-course'}
      <!-- Navigation and content for regular pages -->
      <div class="w-full min-h-screen bg-gradient-light dark:bg-gradient-dark">
