  ...
  try {
    ...
    if (ytInitialDataMatch) {
      try {
        ...
        const videoDetails = ytInitialData?.videoDetails || ytInitialData?.contents?.twoColumnWatchNextResults?.results?.results?.contents?.[0]?.videoPrimaryInfoRenderer;
        if (videoDetails) {
      ...
      for (const pattern of durationPatterns) {
        ...
        if (match) {
          if (match[2]) {
            ...
            durationSeconds = Math.round(parseInt(match[1]) / 1e3);
          } else {
